...
Git hooks live in `.git/hooks/` which is not tracked by git. Claude Code settings and Codex CLI configs are also local-only. After cloning a harness project, re-install all three surfaces:
...
- `.codex/config.toml` — Codex CLI notification hook config
- `.codex/rules/guardian.rules` — Codex CLI execpolicy rules

Claude Code settings (`.claude/settings.local.json`) are regenerated by running the full scaffold setup.
