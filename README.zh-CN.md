...
这个仓库当前聚焦于已发布的 `harness-engineering-orchestrator` skill，以及围绕它的安装、验证和贡献说明。
...
当前仓库根目录不再保留 `AGENTS.md` 或 `CLAUDE.md`。如果目标项目需要这些运行时指令文件，`harness-engineering-orchestrator` 会在 setup 阶段按需生成。
