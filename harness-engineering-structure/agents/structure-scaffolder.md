...
3. Add CI workflows (`.github/workflows/`) for validation, testing, and release gating.
4. Create documentation scaffolds in `docs/` (product.md, architecture.md, progress.md).
5. Generate `AGENTS.md` with agent dispatch guidance and `CLAUDE.md` with project-specific instructions.
6. Install git hooks for pre-commit validation and pre-push checks.
7. Run `bun run harness:doctor` to verify the scaffolded project is healthy.

### Hydration: add harness to existing repo
1. Add `harness/` directory containing `config.json` and rules, without overwriting any existing files.
...
4. Add `AGENTS.md` and `CLAUDE.md` only if they do not already exist.
